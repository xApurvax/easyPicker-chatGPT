...
const HeaderAfterAuth = ({children}) => {
  ...
  useEffect(() => {
    if((domRef.current.clientHeight / 10 ) - 15 > 90 && (domRef.current.clientHeight / 10 ) - 15 < 150 ){
      document.getElementById("headerAaHRP").style.paddingTop = String(Math.round((domRef.current.clientHeight / 10 ) - 15) + "px");
    }
  }, [])
